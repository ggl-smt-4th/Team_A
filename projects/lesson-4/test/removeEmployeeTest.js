...
contract('Payroll', (accounts) => {
  ...
  it("Test remove nonexisted employee by owner", () => {
    // Remove employee
    return payroll.removeEmployee(guest, {from: owner}).then(() => {
    assert(false, "Should not be successful");
  }).catch(error => {
    assert.include(error.toString(), "Error: VM Exception", "Cannot remove nonexisted employee by owner");
  })
  });
  ...
  it("Test remove nonexisted employee by guest", () => {
    // Remove employee
    return payroll.removeEmployee(guest, {from: guest}).then(() => {
    assert(false, "Should not be successful");
  }).catch(error => {
    assert.include(error.toString(), "Error: VM Exception", "Cannot remove nonexisted employee by guest");
  })
 });
